<template>
    <div class="about">
        <h1>文章列表</h1>
        <!-- <el-row class="demo-autocomplete">
          <el-col :span="12">
            <el-autocomplete
              class="inline-input"
              v-model="state2"
              :fetch-suggestions="querySearch"
              placeholder="请输入内容"
              :trigger-on-focus="false"
              @select="handleSelect"
            ></el-autocomplete>
          </el-col>
        </el-row> -->
        <el-table :data="items">
        <el-table-column prop="_id" label="ID" width="230"> </el-table-column>
        <!-- <el-table-column prop="parent.name" label="上级分类"></el-table-column> -->
        <el-table-column prop="title" label="标题"></el-table-column>
        <el-table-column prop="categories" label="分类">
          <template slot-scope="scope">
            <span v-for="(item, index) in scope.row.categories" :key="index">
              <el-tag size="mini" style="margin-right:5px">{{item.name}}</el-tag>
            </span>
          </template>
        </el-table-column>
        <el-table-column prop="tags.name" label="标签">
          <template slot-scope="scope">
            <!-- <span v-for="(item, index) in scope.row.tags" :key="index">
              <el-tag size="mini">{{item.name}}</el-tag>
            </span> -->
            <el-tag size="mini" v-for="(item,index) in scope.row.tags" :key="index" style="margin-right:5px">{{item.name}}</el-tag>
          </template>
        </el-table-column>
        <el-table-column fixed="right" label="操作" width="180">
        <template slot-scope="scope">
            <!-- <el-button @click="handleClick(scope.row)" type="text" size="small">查看</el-button> -->
            <el-button type="primary" size="small" @click="$router.push(`/articles/edit/${scope.row._id}`)">编辑</el-button>
            <el-button type="danger" size="small" @click="remove(scope.row)">删除</el-button>
        </template>
        </el-table-column>
      </el-table>
    </div>
</template>
<script>
export default {
    data(){
        return{
            items:[],
            // restaurants: [{ text: 'JS1', value: 'JS1' }, { text: 'HTML', value: 'HTML' }],//标签
            // categoriesName:[],
            // tagsName:[]
        }
    },
    methods:{
      formatter(row, column) {
        return row.address;
      },
      async filterTag(value, row) {
        // console.log(value)
        // const res = await this.$http.get('rest/articles')
        // // console.log(res.data.tags._id);
        // let len=res.data.length;
        // console.log(len)
        // for(let i=0;i<len;i++){
        //   for(let j=0;j<res.data[i].tags.length;j++){
        //     return (res.data[i].tags[j].name)===value;
        //   }
        // }
      },
        async fetch(){
            const res = await this.$http.get('rest/articles')
            this.items = res.data;
        },
        async remove(row){
            this.$confirm(`是否要删除文章"${row.title}"`, '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
            }).then( async () => {
                const res = await this.$http.delete(`rest/articles/${row._id}`)
                this.$message({
                    type: 'success',
                    message: '删除成功!'
                });
                this.fetch()
            })
        }

    },
    created(){
        this.fetch()
    }
}
</script>